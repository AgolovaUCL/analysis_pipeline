...
def makefig(angle, radius, hcoord_translated, vcoord_translated, img, output_path):# Create the figure and axis
    ...
    for i, (x, y) in enumerate(zip(hcoord_translated, vcoord_translated)):
        ...
        ax.text(x, y, i + 1, ha='center', va='center', size=10, c = 'r')  # Start numbering from 1
        ax.add_patch(hex)
...
def get_translated_coords(hcoord2, vcoord2, theta, desired_x, desired_y):
    ...
    dx = desired_x - hcoord_rotated[63]
    dy = desired_y - vcoord_rotated[63]
...
def overlay_maze_image_consinks(derivatives_base, method):
    """
    ...
    # Output folder
    output_path = os.path.join(derivatives_base, "analysis", "maze_overlay", "maze_overlay_consinks.png")
