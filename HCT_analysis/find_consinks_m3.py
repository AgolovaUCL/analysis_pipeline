...
def rel_dir_distribution_all_sinks(spike_train, reldir_occ_by_pos, sink_bins, candidate_sinks, direction_bins, pos_data):
        ...
        for j in range(np.max(y_bin)+1):
            ...
            for i2, x_sink in enumerate(candidate_sinks['x']):
                for j2, y_sink in enumerate(candidate_sinks['y']):
                    ...
                    rel_dir_binned_counts_normalised = rel_dir_binned_counts/reldir_occ_by_pos[j, i, j2, i, :]
                    rel_dir_dist[j2, i2, :] = rel_dir_dist[j2, i2, :] + rel_dir_binned_counts_normalised
...
def find_consink(spike_train, reldir_occ_by_pos, sink_bins, direction_bins, candidate_sinks, pos_data):
    ...
    rel_dir_dist = rel_dir_distribution_all_sinks(spike_train, reldir_occ_by_pos,  sink_bins, candidate_sinks, direction_bins, pos_data)

    # calculate the mean resultant length of the normalised relative direction distribution
    mrl, mean_angle = mean_resultant_length_nrdd(rel_dir_dist, direction_bins)
