...
def get_directional_occupancy_by_position(dlc_data, limits, frame_rate=25):
    ...
    # get the durations
    durations = np.ones(len(hd))/frame_rate
...
def main(derivatives_base, rawsession_folder, xy_lims, code_to_run = []):
    """
    ...
    if 1 in code_to_run:
        ...
        for g in [1,2]:
            spike_rates_by_position_and_direction_goal = bin_spikes_by_position_and_direction_individual_units(unit_ids, sorting, rawsession_folder,
                                                    directional_occupancy_by_position, pos_data, goal=g)
            # save the spike rates by position and direction
...
if __name__ == "__main__":
    ...
    rawsession_folder = r"D:\Spatiotemporal_task\rawdata\sub-003_id_2V\ses-02_testHCT"
