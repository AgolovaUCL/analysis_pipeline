...
def savepositions(position, output_path, name):
        midpoint_ears = position.sel(keypoints=["left_ear", "right_ear"]).mean(dim="keypoints")
...
def run_movement():
    ...
    special_positional_data_folder = os.path.join(positional_data_folder, 't1_tests')
    if not os.path.exists(special_positional_data_folder):
        os.makedirs(special_positional_data_folder)
    ...
    for tr in trial_numbers:
        ...
        savepositions(position, special_positional_data_folder, "t1_XY_raw.csv")
        ...
        position = ds.position
        savepositions(position, special_positional_data_folder, "t1_XY_after_intp.csv")
        ...
        savepositions(position, special_positional_data_folder, "t1_XY_smoothed_w90.csv")
        ...
        #fig.savefig(output_path, dpi=300, bbox_inches="tight")
        #plot_raw_and_smooth_timeseries_and_psd(ds, ds_smooth, keypoint="left_ear")
        ...
        position = ds_smooth.position
        savepositions(position, special_positional_data_folder, "t1_XY_smoothed_w30.csv")
        g = ds_smooth.position.squeeze().plot.line(
            x="time", row="keypoints", hue="space", aspect=2, size=2.5
        )
        fig = g.fig

        window = int(0.5*ds.fps)
        ds_smooth = ds.copy()
        ds_smooth.update(
            {
                "position": rolling_filter(
        ...
        position = ds_smooth.position
        savepositions(position, special_positional_data_folder, "t1_XY_smoothed_w15.csv")
        g = ds_smooth.position.squeeze().plot.line(
            x="time", row="keypoints", hue="space", aspect=2, size=2.5
        )
        fig = g.fig
