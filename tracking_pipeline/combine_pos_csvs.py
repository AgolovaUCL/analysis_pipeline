...
def get_trial_length(trials_length, total_length_s, tr, frame_rate):
...
def get_df_trial(folder_path, tr, length_fr, method):
...
def get_padding(df_tr, length_fr):
...
def save_df(df, folder_path, method):
    ...
    if method=="ears":
        ...
        output_path = os.path.join(folder_path, "XY_HD_alltrials_center.csv")
        
    df.to_csv(output_path, index = False)
    print(f"Dataframe saved to {output_path}")

def check_difference_df(df, total_length_s,  frame_rate):
