...
import json
from find_consinks_platformbins import find_consink, find_consink_method2, get_dir_allframes, recalculate_consink_to_all_candidates_from_translation
...
def plot_consinks_interactive(derivatives_base, unit_id, rel_dir_occ = "intervals",  include_g0 = True,  frame_rate=25, sample_rate=30000):
    """
    ...
    while input != 'q':
        # Make plot
        fig, axs = plt.subplots(len(goals), 4, figsize=[18, len(goals)*5], constrained_layout=True)
        ...
        for i, g in tqdm(enumerate(goals)): # We make separate plot for each of them
            ...
            # Third, consink plots
            # For within
            for m in [1,2]: # seperating for method 1 and method 2
                ...
                ci = recalculate_consink_to_all_candidates_from_translation(spike_train_filt, pos_data,
                                                                            ...
                                                                            intervals_frames, method = m, goal=g)
